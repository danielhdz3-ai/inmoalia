    ...
    <Link href={`/productos/${product.slug}`} className="group block">
      <div className="relative overflow-hidden rounded-xl bg-[#f9f6f1] border border-[#e8ddd0] hover:border-[#d4c4b0] transition-all duration-200 hover:shadow-md">
        ...
        >
          ...
          {/* Details overlay - MEJORADO */}
          {showDetails && (
            <div className="absolute inset-0 bg-gradient-to-t from-black/95 via-black/70 to-transparent flex flex-col justify-end p-4 transition-opacity duration-300">
              <div className="text-white space-y-2">
                {/* SKU y Stock */}
                <div className="flex items-center justify-between text-[10px] font-medium">
                  {product.sku && (
                    <span className="bg-white/20 backdrop-blur-sm px-2 py-0.5 rounded">
                      SKU: {product.sku}
                    </span>
                  )}
                  {product.stock > 0 ? (
                ...
                <div className="space-y-1">
                  {dimensionText && (
                    <div className="flex items-start gap-1.5">
                      ...
                      <span className="text-xs font-medium">Dimensiones: {dimensionText}</span>
                    </div>
                  )}
                  ...
                  {product.material && (
                    <div className="flex items-start gap-1.5">
                      ...
                      <span className="text-xs">Material: {product.material}</span>
                    </div>
                  )}
                ...
                {/* Descripción */}
                {product.description && (
                  <p className="text-xs leading-relaxed line-clamp-3 opacity-90 border-t border-white/20 pt-2">
                    {product.description}
                  </p>
                )}
                ...
                {product.tags && product.tags.length > 0 && (
                  <div className="flex flex-wrap gap-1 pt-1">
                    {product.tags.slice(0, 4).map((tag, idx) => (
                      <span
                        key={idx}
                        className="text-[9px] bg-white/15 backdrop-blur-sm px-1.5 py-0.5 rounded-sm"
                      >
                        {tag}
                      </span>
                    ))}
                ...
                <div className="text-center pt-2 border-t border-white/20">
                  <span className="text-[10px] opacity-75">Click para ver más detalles →</span>
                </div>
              </div>
            </div>
          )}
